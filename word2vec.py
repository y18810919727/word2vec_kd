    ...
    def create_kd_tree_node(self,data_list,codes,points,inner_id):
        ...
        for d in range(self.new_dimension):
            ...
            if tmp_var > max_var :
                best_split = d
        '''
        logger.info("It's inner.")
        logger.info(codes)
        logger.info(points)
        '''
        data_list = sorted(data_list,key= lambda x:x.vec[best_split])
        new_points = array(list(points) + [inner_id], dtype=uint32)
        inner_id += 1
        (left_inner_num, left_dep) = self.create_kd_tree_node(data_list[0:len(data_list)//2],array(list(codes)+[0],dtype=uint8),new_points,inner_id)
        inner_id += left_inner_num
        (right_inner_num, right_dep) = self.create_kd_tree_node(data_list[len(data_list)//2:],array(list(codes)+[1],dtype=uint8),new_points,inner_id)
